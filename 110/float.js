...
function calculate_float(input) {
  ...
  if (binary_input.length > 9) {
    outputs[0].innerHTML = `${input} in binary is ${sign}<span style='color: aquamarine'>${binary_input.substring(
      0,
      10
    )}</span> <span style='color: lightcoral' title='Cannot express in a finite number of bits.'>[Rounded]</span>`;
  } else
    outputs[0].innerHTML = `${input} in binary is ${sign}<span style='color: aquamarine'>${binary_input}</span>`;

  // standard form conversion
  standard_input = binary_input.substring(0, 10);
  var exponent = 0,
    decimal_point_index = standard_input.indexOf('.');

  if (standard_input == '0') exponent = 0;
  else if (parseFloat(standard_input) < 1 && decimal_point_index != -1) {
    // cases like 0.01, 0.625 [small floats]
    let decimal_string = standard_input.substring(decimal_point_index + 1),
      end_index = decimal_string.indexOf('1');
    if (end_index != -1)
      decimal_string = decimal_string.substring(0, end_index);
    exponent = -decimal_string.length;
  } else if (decimal_point_index != -1) {
    // cases like 101.1, 100111.1 [large floats]
  ...
  if (decimal_point_index != -1)
    standard_input =
      standard_input.substring(0, decimal_point_index) +
      standard_input.substring(decimal_point_index + 1);
  if (standard_input.indexOf('1') != -1)
    standard_input = standard_input.substring(standard_input.indexOf('1'));
  if (standard_input.length > 9) {
    standard_input = standard_input.substring(0, 10);
    var rounded =
      "<span style='color: lightcoral' title='Cannot express accurately in 9 bits.'>[Rounded]</span>";
  } else var rounded = '';
  ...
  if (standard_input.length < 9)
    var mantissa = (standard_input + '00000000').substring(0, 9);
  ...
  if (exponent.toString().length < 5) {
    var exponent = '0000' + exponent.toString(2);
    exponent = exponent.substring(exponent.length - 5);
