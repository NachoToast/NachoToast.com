...
// right user check
session_start();
if ($_SESSION["id"] != $_GET["user"]) {header("location: ../profiles.php?user=$redirect&e=na"); exit();}
...
if (isset($_POST["uc_submit"]) && isset($_POST["new_username"])) { // change username
    ...
    // empty
    if (empty($username)) {header("location: ../profiles.php?user=$redirect&e=username_empty"); exit();}

    // invalid
    if (!preg_match("/^[a-zA-Z0-9]*$/", $username) || strlen($username) > 20 || strlen($username) < 3) {header("location: ../profiles.php?user=$redirect&e=username_invalid"); exit();}
    ...
    if ($result > 0) {header("location: ../profiles.php?user=$redirect&e=username_taken"); exit();}
    ...
    if (time() - $result["usernamechanged"] < 259200 || time() - $result["registered"] < 259200) {header("location: ../profiles.php?user=$redirect&e=username_toosoon"); exit();
    }
    ...
    $sql -> execute();
    header("location: ../profiles.php?user=$redirect&s=username");
    exit();
    ...
    if (strlen($new) > 255 || strlen($new) > 255 || $new !== $new2 || empty($new)) {header("location: ../profiles.php?user=$redirect&e=password_invalid"); exit();}
    ...
    if (!password_verify($old, $online_hashed)) {header("location: ../profiles.php?user=$redirect&e=password_incorrect"); exit();}
    ...
    $sql -> execute();
    header("location: ../profiles.php?user=$redirect&s=password");
    exit();
    ...
    if (getimagesize($_FILES["new_pfp"]["tmp_name"]) == false) {
        header("location: ../profiles.php?user=$redirect&e=pfp_bad");
        exit();
    }

    // size (2mb)
    if ($_FILES["new_pfp"]["size"] > 2000000) {
        header("location: ../profiles.php?user=$redirect&e=pfp_toobig");
    }

    // extension
    if ($file_extension !== "jpg" && $file_extension !== "png" && $file_extension !== "jpeg") {
        header("location: ../profiles.php?user=$redirect&e=pfp_invalid");
        exit();
    }
    ...
    if (move_uploaded_file($_FILES["new_pfp"]["tmp_name"], $new_file_name)) {
        ...
        if ($db_cpfp == 1 && $db_ext !== $file_extension) { // delete previous if extension different
            if (!unlink($base_directory . $_SESSION["id"] . "." . $db_ext)) {
                header("location: ../profiles.php?user=$redirect&e=pfp_deleteold");
                exit();
            }
        }
        ...
        header("location: ../profiles.php?user=$redirect&s=pfp");
        exit();
    } else { // move failure
        header("location: ../profiles.php?user=$redirect&e=pfp_move");
        exit();
    }
} else {
    header("location: ../profiles.php?user=$redirect");
    exit();
}